...
async function fetchData<T extends BaseRecord>(table: string): Promise<T[]> {
	...
	const formattedData = data?.map(item => {
		const { created_at, updated_at, image_url, ticket_url, ...rest } = item
...
async function updateData<T extends BaseRecord>(
	table: string,
	items: T[]
): Promise<T[]> {
	if (!isSupabaseConfigured()) {
	...
	const formattedItems = items.map(item => {
		const { createdAt, updatedAt, imageUrl, ticketUrl, ...rest } = item
...
export async function updateSpotifyLinks(
	links: MusicLink[]
): Promise<MusicLink[]> {
...
export async function updateSoundCloudLinks(
	links: MusicLink[]
): Promise<MusicLink[]> {
	const updatedLinks = links.map(link => ({
		...link,
...
export async function updateCommunityPosts(
	posts: CommunityPost[]
