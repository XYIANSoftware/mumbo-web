...
interface Field {
	field: string
	header: string
	type: 'text' | 'date' | 'time' | 'dropdown' | 'textarea' | 'image'
...
export function EditableList<T extends BaseContent>({
			...
			case 'time':
				return (
					<Calendar
						...
						onChange={e =>
							onChange(
								e.value?.toLocaleTimeString('en-US', { hour12: false }) || ''
							)
						}
						timeOnly
						className='w-full'
	...
	const bodyTemplate = (rowData: T, field: Field) => {
		if (editingItem?.id === rowData.id) {
			return renderEditor(
				...
				(value, path) => {
					const updates = {
						[field.field]: value,
						...(field.type === 'image' && path !== undefined
							? { image_path: path, image_url: value }
							: {}),
					} as Partial<T>

					const updatedItem = { ...rowData, ...updates }
					setItems(
		...
		if (field.type === 'image' && rowData[field.field as keyof T]) {
			return (
				<div className='w-24 h-24'>
					<img
						src={rowData[field.field as keyof T]?.toString()}
						alt='Preview'
