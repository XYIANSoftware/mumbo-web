...
export function SoundCloudLinksEditor() {
  ...
  const fetchItems = async () => {
    try {
      const response = await fetch('/api/admin/soundcloud-links');
      if (!response.ok) throw new Error('Failed to fetch SoundCloud links');
      ...
      toast.current?.show({
        severity: 'error',
        summary: 'Error',
        detail: 'Failed to fetch SoundCloud links',
        life: 3000,
      });
    } finally {
      setLoading(false);
  ...
  const handleSave = async (items: MusicLink[]) => {
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to save SoundCloud links');
      }
      ...
      toast.current?.show({
        severity: 'success',
        summary: 'Success',
        detail: 'SoundCloud links saved successfully',
        life: 3000,
      });
    } catch (error) {
      ...
      toast.current?.show({
        severity: 'error',
        summary: 'Error',
        detail: 'Failed to save SoundCloud links',
        life: 3000,
      });
      throw error;
    }
